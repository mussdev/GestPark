...
{
    public partial class FormImportCleanCar : Form
    {
        private Guid IdVehiWatch;
        private SqlDataReader MyReader;
        public FormImportCleanCar()
        {
            InitializeComponent();
        }
        

        private void ExporterCleanCar()
        {
        ...
        private void btnValImpCleanCar_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    if (Conn.IsConnection)
                    {
                        using (var Cmd = Conn.cn.CreateCommand())
                        {
                            for (int i = 0; i < dgvImpCleanCar.Rows.Count; i++)
                            {
                                ...
                                Cmd.CommandText = "INSERT INTO LAVAGE (ID_VEHICULE,CODE_WTCH,KILO_AVANT_VIDENGE,MONTANT_PROV,DATE_PROV,LITRE_PROV,PRIX_LITRE_PROV) VALUES (@ID_VEHICULE,@CODE_PROV,@KILO_AVANT_VIDENGE,@MONTANT_PROV,@DATE_PROV,@LITRE_PROV,@PRIX_LITRE_PROV)";
                                Cmd.Parameters.AddWithValue("@ID_VEHICULE", IdVehiWatch);
                                Cmd.Parameters.AddWithValue("@CODE_WTCH", dgvImpCleanCar.Rows[i].Cells["CODE_WTCH"].Value.ToString());
                                Cmd.Parameters.AddWithValue("@MONTANT_WTCH", dgvImpCleanCar.Rows[i].Cells["MONTANT_WTCH"].Value.ToString());
                                Cmd.Parameters.AddWithValue("@LIEU_WTCH", dgvImpCleanCar.Rows[i].Cells["LIEU_WTCH"].Value.ToString());
                                Cmd.Parameters.AddWithValue("@DATEREGISTER", dgvImpCleanCar.Rows[i].Cells["DATEREGISTER"].Value.ToString());
                                Cmd.Parameters.AddWithValue("@DATECREATE_WTCH", dgvImpCleanCar.Rows[i].Cells["DATECREATE_WTCH"].Value.ToString());
                                Cmd.ExecuteNonQuery();
                            }
                        }
                        Conn.cn.Close();
        ...
        private void btnLoadImpCleanCar_Click(object sender, EventArgs e)
        {
        ...
        private void btnExpCleanCar_Click(object sender, EventArgs e)
        {
            ExporterCleanCar();
        }
    }
